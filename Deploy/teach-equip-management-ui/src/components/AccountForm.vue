...
const validateInputs = async () => {
  ...
  if (isProcess) {
    ...
    const response = fetch("http://localhost:8080/api/usermanage/create-user", {
      method: "POST",
      headers: {
  ...
  try {
    ...
    const newUserDetail = {
      ...
      const response = await axios.post(
        "http://localhost:8080/api/usermanage/create-user-detail",
        newUserDetail,
        {
          headers: {
      ...
      await axios.delete(
        `http://localhost:8080/api/usermanage/remove-account/${accountId}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
