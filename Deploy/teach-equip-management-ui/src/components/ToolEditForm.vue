...
const validateInputs = async () => {
  ...
  if (isProcess) {
    ...
    const response = fetch("http://localhost:8080/api/toolmanage/update-tool", {
      method: "PUT",
      headers: {
...
const removeToolCategory = async (toolId, categoryId) => {
  try {
    ...
    const response = await axios.delete(
      "http://localhost:8080/api/toolmanage/remove-tool-category",
      {
        headers: {
          Authorization: "Bearer " + localStorage.getItem("access_token"),
        },
...
const toolById = async (itemId) => {
  try {
    const response = await axios.get(
      `http://localhost:8080/api/toolmanage/tool/find/${itemId}`,
      {
        headers: {
          Authorization: "Bearer " + localStorage.getItem("access_token"),
        },
